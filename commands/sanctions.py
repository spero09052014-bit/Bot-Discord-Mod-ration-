...
class Sanctions(commands.Cog):
    ...
    # ------------------------------------------------------------------
    @commands.hybrid_command(name="mute", help="Rend muet un membre pendant une durée donnée. Ex : !mute @membre 10m Spam")
    ...
    async def mute(self, ctx: commands.Context, membre: discord.Member, duree: str, *, raison: str = "Aucune raison précisée"):
        ...
        await _modlog.envoyer_log(
            self.bot, id_serveur=ctx.guild.id, titre="🔇 Mute", description=f"{membre.mention} muté par {ctx.author.mention}",
    ...
    # ------------------------------------------------------------------
    @commands.hybrid_command(name="unmute", help="Retire la sourdine d'un membre.")
    @commands.has_permissions(moderate_members=True)
    @commands.bot_has_permissions(moderate_members=True)
    @commands.guild_only()
    async def unmute(self, ctx: commands.Context, membre: discord.Member, *, raison: str = "Aucune raison précisée"):
        ...
        await _modlog.envoyer_log(
            self.bot, id_serveur=ctx.guild.id, titre="🔊 Unmute", description=f"{membre.mention} démuté par {ctx.author.mention}",
    ...
    # !kick
    # ------------------------------------------------------------------
    @commands.hybrid_command(name="kick", help="Expulse un membre du serveur.")
    ...
    async def kick(self, ctx: commands.Context, membre: discord.Member, *, raison: str = "Aucune raison précisée"):
        ...
        await _modlog.envoyer_log(
            self.bot, id_serveur=ctx.guild.id, titre="👢 Kick", description=f"{membre} expulsé par {ctx.author.mention}",
    ...
    # !ban
    # ------------------------------------------------------------------
    @commands.hybrid_command(name="ban", help="Bannit un membre du serveur.")
    ...
    async def ban(self, ctx: commands.Context, membre: discord.Member, *, raison: str = "Aucune raison précisée"):
        ...
        await _modlog.envoyer_log(
            self.bot, id_serveur=ctx.guild.id, titre="🔨 Ban", description=f"{membre} banni par {ctx.author.mention}",
    ...
    # ------------------------------------------------------------------
    @commands.hybrid_command(name="unban", help="Débannit un utilisateur via son ID Discord.")
    @commands.has_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
    @commands.guild_only()
    async def unban(self, ctx: commands.Context, id_utilisateur: int, *, raison: str = "Aucune raison précisée"):
        ...
        await _modlog.envoyer_log(
            self.bot, id_serveur=ctx.guild.id, titre="✅ Unban", description=f"Utilisateur `{id_utilisateur}` débanni par {ctx.author.mention}",
            couleur=discord.Color.green(), champs={"Raison": raison},
        )
